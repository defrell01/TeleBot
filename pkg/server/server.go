...
func (s *AuthorizationServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	requestToken, err := s.tokenRepository.Get(uint64(chatID), repository.RequestTokens)
	if err != nil {
		w.WriteHeader(http.StatusUnauthorized)
		return
	}

	authReponce, err := s.pocketClient.Authorize(r.Context(), requestToken)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	err = s.tokenRepository.Save(uint64(chatID), authReponce.AccessToken, repository.AccessTokens)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	log.Printf("chat_id: %d\nrequest_token: %s\naccess_token: %s\n", chatID, requestToken, authReponce.AccessToken)
